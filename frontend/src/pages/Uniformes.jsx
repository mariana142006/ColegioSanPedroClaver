...
const cargarUniformes = async () => {
  ...
  const obtenerAlertas = () => {
    ...
    return Object.values(grupos)
      ...
      .map((registros) => {
        const item = registros[registros.length - 1];
        ...
        return {
          ...
          grupo_alerta: Number(item.grupo_alerta),
  ...
  const notificarAcudienteWhatsApp = async (item) => {
    ...
    const mensaje =
      `cordial saludo senor o senora ${item.nombre_acudiente || "acudiente"}, ` +
      `nos permitimos informarle que el estudiante ${item.nombres}, ` +
      `del grado, ${item.grado}, ha acumulado ${item.total_uniforme} reportes del uso inadecuado del uniforme, ` +
    ...
    try {
      ...
      await api.post("/cartas", {
        ...
        grupo_alerta: Number(item.grupo_alerta || 0),
        tipo: "uniforme",
              ...
              <tr key={item.id_uniforme}>
                ...
                <td>
                  {(() => {
                    ...
                    const registrosGrupo = uniformes.filter(
                      (registro) =>
                    ...
                    if (cartaGenerada || whatsappNotificado) {
                      return (
